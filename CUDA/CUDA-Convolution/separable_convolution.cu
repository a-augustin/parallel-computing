...
void compute_on_device_optimized(float *gpu_result, float *matrix_c, float *kernel, int num_cols, int num_rows)
{
    ...
    gettimeofday(&start, NULL);
    convolve_rows_kernel_optimized<<<grid, thread_block>>>(d_row_col, d_matrix, num_rows, num_cols);
    cudaDeviceSynchronize();
    convolve_columns_kernel_optimized<<<grid, thread_block>>>(d_gpu_result, d_row_col, num_rows, num_cols);
    cudaDeviceSynchronize();
    gettimeofday(&stop, NULL);
